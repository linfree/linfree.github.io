...
<!--20120214版-->
<?php
date_default_timezone_set("PRC");if ($_POST) {
	$data = NULL!==addslashes(trim($_POST['what'])) ? addslashes(trim($_POST['what'])) :'' ;

$data = mb_substr(str_replace(array(''),array(''),$data),0,82,'gb2312');}
if (!empty($data))
{
...
fwrite($file,$text);
fclose($file);
echo "<script>setTimeout(function(){window.location='http://127.0.0.1/juhua.php';},2000); </script>";
}
?>
<head">
...
<textarea rows="5" id="what" style="font-family:Times New Roman;font-size:14pt;" cols="80" name="what" placeholder="说点什么吧?" ></textarea>
<p class="tx">Chating Room is Powered By <a href="http://blackbap.org" target="_blank">Silic Group Hacker Army</a>&copy;2009-2011</p>
<input type="submit" value="雁过留声 路过留言" tilte="提交" style="width:120px;height:64px; nl" onsubmit="history.go(0);">
</form>
</center>
